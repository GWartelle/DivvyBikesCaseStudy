...
- `Data/` has the data used for this study in two folders. First in `Chicago Shapefile/` there is the shapefile used on [Tableau](https://www.tableau.com/) for creating the heatmaps, and `Study Data/` is where the .csv files used for the study are supposed to go
